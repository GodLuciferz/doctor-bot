...
let userState = {};          // patient flow state
let tokenCounter = {};
let appointments = {};
...
let pendingAdmins = {};
let userLang = {};           // language preference — reset after each appointment
...
// Exact trigger words — full word match only
const triggerWords = [
...
function getPhoneNumber(senderId) {
    return senderId.replace(/@.+/, "").trim();
}

function checkIsSuperAdmin(sender) {
    return SUPERADMIN_PHONES.some(s => sender.includes(s) || s.includes(sender));
}

function checkIsAdmin(sender) {
    if (checkIsSuperAdmin(sender)) return true;
    return dynamicAdmins.some(a => sender.includes(a) || a.includes(sender));
}

// Exact word match — "12 april" mein "appointment" match nahi hoga
function isTriggerWord(text) {
    return triggerWords.includes(text);
}

// 7 din se purane appointments delete karo
function cleanOldAppointments() {
    const sevenDaysAgo = Date.now() - 7 * 24 * 60 * 60 * 1000;
    Object.keys(appointments).forEach(key => {
        if (appointments[key].timestamp < sevenDaysAgo) {
...
client.on("message", async message => {
    ...
    if (text === "join admin") {
        ...
        if (checkIsAdmin(sender))
            return message.reply("Aap pehle se admin hain ✅");
    ...
    if (isSuperAdmin && isLoggedIn) {
        ...
        if (text.startsWith("approve ")) {
            ...
            if (!pendingAdmins[num])
                return message.reply(`${num} ki koi pending request nahi hai`);
            ...
            try {
                await client.sendMessage(num + "@c.us",
                    "🎉 Aapki admin request approve ho gayi!\nAb *login 1234* likh ke admin panel use karein."
                );
            } catch (e) {}
        ...
        if (text.startsWith("disapprove ")) {
            ...
            try {
                await client.sendMessage(num + "@c.us", "❌ Aapki admin request reject kar di gayi.");
            } catch (e) {}
        ...
        if (text === "pending") {
            ...
            if (!list.length)
                return message.reply("Koi pending request nahi hai");
        ...
        if (text.startsWith("admin add ")) {
            ...
            if (dynamicAdmins.includes(newAdmin))
                return message.reply(`${newAdmin} already admin hai ✅`);
        ...
        if (text === "admin list") {
            ...
            reply += "\n👤 Admins:\n";
            reply += dynamicAdmins.length
                ? dynamicAdmins.map(a => `• ${a}`).join("\n")
                : "Koi admin nahi hai";
    ...
    if (isAdmin && isLoggedIn) {
        ...
        if (text === "list") {
            ...
            if (!todayList.length)
                return message.reply("Aaj ki koi appointment nahi hai");

            let reply = "📋 Aaj ki Appointments:\n\n";
            todayList.sort((a, b) => a.token - b.token).forEach(a => {
                reply += `Token: ${a.token}\nName: ${a.name}\nPhone: ${a.phone}\n\n`;
            });
        ...
        if (text === "history") {
            ...
            if (!Object.keys(appointments).length)
                return message.reply("Koi appointment nahi mili (7 din mein)");
            ...
            Object.keys(grouped).sort().forEach(date => {
                reply += `━━━ ${date} ━━━\n`;
                grouped[date].sort((a, b) => a.token - b.token).forEach(a => {
                    reply += `Token ${a.token}: ${a.name} (${a.phone})\n`;
                });
                reply += "\n";
            });
        ...
        if (text === "reset") {
            tokenCounter = {};
            appointments = {};
            return message.reply("Tokens reset successfully ✅");
        }
        ...
        if (text === "close") {
            clinicOpen = false;
            return message.reply("Clinic closed ❌");
        }

        if (text === "open") {
            clinicOpen = true;
            return message.reply("Clinic open ✅");
        }
    ...
    if (!clinicOpen)
        return message.reply(t(senderRaw, "closed"));
    ...
    if (closedDays.includes(todayDay))
        return message.reply(t(senderRaw, "closedDay", todayDay));

    if (closedDates.includes(todayDate))
        return message.reply(t(senderRaw, "closedDate", todayDate));

    /* ===== LANGUAGE PENDING — pehle check karo ===== */

    if (userState[senderRaw] && userState[senderRaw].step === 0) {

        if (text === 'hindi') {
            userLang[senderRaw] = 'hi';
            userState[senderRaw] = { step: 1 };
            return message.reply(msg.hi.welcome);
        }

        if (text === 'english') {
            userLang[senderRaw] = 'en';
            userState[senderRaw] = { step: 1 };
            return message.reply(msg.en.welcome);
        }

        return message.reply(msg.hi.selectLang);
    }

    /* ===== PATIENT FLOW — name/phone/date ===== */
    ...
    if (userState[senderRaw] && userState[senderRaw].step >= 1) {
        ...
        if (userState[senderRaw].step === 3) {
            ...
            if (tokenCounter[date] > dailyLimit) {
    ...
    if (isTriggerWord(text)) {
        userState[senderRaw] = { step: 0 };
        return message.reply(msg.hi.selectLang);
    }
